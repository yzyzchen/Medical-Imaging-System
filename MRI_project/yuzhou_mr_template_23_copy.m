...
figure(6)
plot(xpos,abs(ift(sig_6)));xlabel('x_position');ylabel('received signal');title('magnitude vs spatial position');
...
for pe = 1:npe
    ...
    bz_7 = gx_7 * obj_x + gy_7 * obj_y + gz * obj_z;
    [mx_7,my_7,mz_7] = blochsim_516(m0,bx_7,by_7,bz_7,obj_T1,obj_T2,dt);
    M_7(:,pe) = sum(mx_7,2)+ 1i.*sum(my_7,2);
    index_1 = 1;
    for index = 1 : totalTimepoints
        if (index >= 151 && index <= 310) && mod(index,2) == 1
...
subplot(2,2,3)
imagesc(kxpos,kypos,abs(sig_7)'); colormap gray; axis('image'); axis('xy')
xlabel('kx (cm^-1)');
ylabel('ky (cm^-1)');
title('abs(M(kx,ky))')
...
figure(9)
subplot(2,2,1)
imagesc(kxpos,kypos,real(sig_9)'); colormap gray; axis('image'); axis('xy')
xlabel('kx (cm^-1)');
ylabel('ky (cm^-1)');
title('real(M(kx,ky))')
...
subplot(2,2,2)
imagesc(kxpos,kypos,imag(sig_9)'); colormap gray; axis('image'); axis('xy')
xlabel('kx (cm^-1)');
ylabel('ky (cm^-1)');
title('imag(M(kx,ky))')

subplot(2,2,3)
imagesc(kxpos,kypos,abs(sig_9)'); colormap gray; axis('image'); axis('xy')
xlabel('kx (cm^-1)');
ylabel('ky (cm^-1)');
title('abs(M(kx,ky))')
...
%% part 10 11
load object23.mat
mx_10sum = zeros([totalTimepoints npe]);
my_10sum = zeros([totalTimepoints npe]);
mz_10sum = zeros([totalTimepoints npe]);
